...
def  getexpression4mean(path,genetype):
    types = getALlcluster(path)
    datanames = os.listdir(path)
    datanames.sort()
    df = pd.DataFrame()
    ...
    for t in types:
        ...
        for n in datanames:
            adata=sc.read_h5ad(path+n)
            num.append(round(np.mean(sum(adata[adata.obs["meta.cluster"]==t][:,adata.var["gene_type"]==genetype].X)),2))
        df[t]=num
...
getexpression4mean(path,"protein_coding")
